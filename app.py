...
def process_video_to_audio(video_path):
    # MoviePy သုံးပြီး Audio သီးသန့်ထုတ်ယူခြင်း
    video = VideoFileClip(video_path)
    audio_path = "temp_audio.mp3"
    video.audio.write_audiofile(audio_path, logger=None)
    video.close() # ဖိုင်ကို ပြန်ပိတ်ရန်
    return audio_path
    ...
    if video_file and st.button("Generate English SRT"):
        with st.spinner("AI က ဗီဒီယိုကို နားထောင်နေပါသည်..."):
            ...
            try:
                ...
                # Gemini ဆီ Audio တင်ခြင်း
                gemini_file = genai.upload_file(audio_path, mime_type="audio/mpeg")
                
                # ဖိုင်အဆင်သင့်ဖြစ်အောင် ခဏစောင့်ခြင်း
                while gemini_file.state.name == "PROCESSING":
                    time.sleep(2)
                    gemini_file = genai.get_file(gemini_file.name)

                model = genai.GenerativeModel('gemini-1.5-flash')
                prompt = "Watch the audio and generate a precise English SRT subtitle file with timestamps. Output ONLY the SRT content."
                ...
                genai.delete_file(gemini_file.name) # API ပေါ်ကဖိုင်ကို ဖျက်ခြင်း
